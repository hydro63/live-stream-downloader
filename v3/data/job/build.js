...
const addEntries = async (entries) => {
  ...
  for (const entry of entries.values()) {
    ...
    if (!r.headers.get("Content-Type")) {
      ...
      if (
        ...
        document.getElementById("online-resolve-name").checked
      ) {
        naming += 1;
    ...
    clone.querySelector("[data-id=href]").textContent = clone.querySelector(
      "[data-id=href]"
    ).title = entry.blocked?.value ? entry.blocked.reason : entry.url || "N/A";

    const mpvCommand = `mpv "${entry.url}" --referrer="${args.get("href")}"`;
    clone.querySelector("input[data-id=mpv]").onclick = (e) =>
      navigator.clipboard
        .writeText(mpvCommand)
        .then(() => {
          e.target.value = "Done";
          setTimeout(() => (e.target.value = "MPV"), 750);
        })
        .catch((e) => alert(e.message));

    clone.querySelector("input[data-id=copy]").onclick = (e) =>
      navigator.clipboard
        .writeText(entry.url)
        .then(() => {
          e.target.value = "Done";
  ...
  document.body.dataset.mode = document.querySelector("form .entry")
    ? "ready"
    : "empty";
  document.title =
    'Download a Media from "' + (args.get("title") || "New Tab") + '"';
};
